...
| Relatel (a TDC company) | **Jensen's own** phone provider — cloud switchboard + Finn's and Dennis's mobiles | Main number is landline-type. Menu: 1 customer service · **2 service → Finn's mobile** · 3 sales → Dennis · 4 opening hours. Plan: option 2 → a Danish Twilio number (DECISIONS 2026-09-24). API `dev.relatel.dk`; webhooks under *Company administration → Webhooks* (Contact Center/Unlimited only). Subscription tier unknown | Admin login → from Dennis (ask for a named admin user, not his) — password manager, never the repo |
| Google (service calendar) | Finn's service calendar "Servicebesøg" — **planned, not created** | Free account on the service mailbox (address to confirm; `service@jensenproduction.dk` likely — that domain's mail is at one.com). System access via a service account the calendar is shared with. `docs/plan-service-calendar.md` | service-account key → `.env.local` + Vercel (when built); account password + recovery phone with Jensen |
