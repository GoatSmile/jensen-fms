...
if lsof -nP -iTCP:"$port" -sTCP:LISTEN >/dev/null 2>&1; then
  note "Gates: tsc + tests passed. Build SKIPPED — a dev server is listening on :$port and building would clobber its chunks. Stop it and run \`npm run build\` before you trust this commit."
fi
...
note "Gates green: tsc + tests + build."
