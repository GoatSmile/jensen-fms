...
/**
 * Horizontal scroll container for the settings sub-rail on narrow screens.
 *
 * Exists for one reason: below `md` the rail scrolls sideways, and the active
 * section can sit off-screen. Landing on `?section=phone` showed a rail reading
 * "General · Communication · Accounting" with the current section out of view —
 * you could not see where you were, which is the one job a rail has.
 *
...
export function SubRailScroller({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div
      ref={ref}
      // Scrollbar hidden: the rail is five short pills, so a visible bar reads
      // as chrome rather than as an affordance.
