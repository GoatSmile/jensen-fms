...
export function PhotoThumb({
  ...
  function runDelete() {
    onError(null);
    start(async () => {
      const r = await deleteBikeImage(bikeId, photo.id);
      if (!r.ok) {
        onError(r.error);
  ...
  return (
    <div className="group bg-muted/30 relative overflow-hidden rounded-md border">
      {/* eslint-disable-next-line @next/next/no-img-element -- Bike photos are
          already client-resized to 1600px WebP; next/image's optimization
